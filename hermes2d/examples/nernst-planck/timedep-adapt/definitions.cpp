...
class CustomWeakFormNernstPlanckCranic : public WeakForm {
public:
  CustomWeakFormNernstPlanckCranic(double* tau, scalar C0, double lin_force_coup, double mech_lambda, double mech_mu, double K, double L, double D, Solution* C_prev_time, Solution* phi_prev_time) : WeakForm(2) {
    for(unsigned int i = 0; i < 2; i++) {
      CustomWeakFormNernstPlanckCranic::VectorFormVol* vector_form = new CustomWeakFormNernstPlanckCranic::VectorFormVol(i, tau, C0, lin_force_coup, mech_lambda, mech_mu, K, L, D);
      if(i == 0) {
      ...
      for(unsigned int j = 0; j < 2; j++)
        add_matrix_form(new CustomWeakFormNernstPlanckCranic::MatrixFormVol(i, j, tau, C0, lin_force_coup, mech_lambda, mech_mu, K, L, D));
    }
  };
